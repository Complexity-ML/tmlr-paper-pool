...
\paragraph{R\'{e}sultats pr\'{e}liminaires (500M tokens).} Sur 954 \'{e}tapes, le Token-Routed converge plus rapidement (perte moyenne 4,793 vs 4,905 pour le dense --- avantage de $-0,112$). Le dense rattrape apr\`{e}s l'\'{e}tape 600 gr\^{a}ce \`{a} sa plus grande capacit\'{e} par token (2416 vs 1024 dims actifs). Le run complet \`{a} 8B tokens d\'{e}terminera si le dense sature (sur-entra\^{i}n\'{e} \`{a} 242\% Chinchilla) tandis que le Token-Routed --- dont chaque expert ne voit que 25\% des tokens --- continue de descendre.

\begin{table}[h]
\centering
\caption{R\'{e}sultats de l'ablation (500M tokens, 954 \'{e}tapes). Le Token-Routed m\`{e}ne sur les 600 premi\`{e}res \'{e}tapes.}
\label{tab:ablation_results}
\begin{tabular}{lccc}
\toprule
\textbf{Run} & \textbf{Perte moy. (954 \'{e}tapes)} & \textbf{Perte finale} & \textbf{PPL finale} \\
\midrule
run1 (Dense 170M)    & 4,905 & 4,064 & 58,2 \\
run2 (Full Complexity 176M)   & \textbf{4,793} & 4,150 & 63,5 \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{Courbes de perte : Dense 170M vs Token-Routed + Shared 176M (500M tokens). Le TR converge plus vite sur les 600 premi\`{e}res \'{e}tapes.}
