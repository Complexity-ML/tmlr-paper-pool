...
attention. A stricter local control also replaces the fixed lookup with a
learned contextual top-2 router while preserving the common backbone, shared
path, experts, data order, optimizer, and token budget. Fixed token-ID routing
has lower NLL than both controls in all four local runs, but these pilots
recycle a small local corpus and are not evidence of scaling or
...
with grouped-query attention (GQA) and multi-head attention (MHA), and whether
the local result survives a direct learned contextual-router control.
...
8B-token experiment. Dense and fixed-routing runs have 99,487,680 parameters,
hidden size 384, 10 layers, eight query heads, and a 200,019-token o200k
vocabulary. GQA uses two key/value heads; MHA uses eight. Each run trains for
250 steps with batch size 4 and sequence length 1024, totaling 1,024,000
...
For the direct routing control, the fixed lookup is replaced by a learned
linear map from the contextual hidden state to four expert logits. Both
variants select two experts. The learned variant uses normalized softmax
weights over the selected experts and a Switch-style balancing loss with
coefficient 0.01. The shared path and expert tensors have the same shapes and
paired initialization as the fixed variant. The learned projection adds 1,536
parameters per layer, or 15,360 total, giving 99,503,040 parameters: a 0.0154\%
increase over the fixed and dense references.

\begin{table}[t]
\centering
\footnotesize
\setlength{\tabcolsep}{3pt}
\caption{\textbf{Matched local Apple-MPS pilots.} Lower NLL is better.
The two seeds use the same split but different initialization and evaluation
sampling seeds.}
\label{tab:mps}
\begin{tabular}{llrrr}
\toprule
Attention & Seed & Dense & Learned & Fixed ID \\
\midrule
GQA & 42 & 7.596686 & 7.602109 & \textbf{7.536167} \\
GQA & 43 & 7.530082 & 7.548665 & \textbf{7.492290} \\
MHA & 42 & 7.586145 & 7.592847 & \textbf{7.536471} \\
MHA & 43 & 7.726971 & 7.666093 & \textbf{7.541488} \\
\bottomrule
\end{tabular}
\end{table}

Fixed token-ID routing has lower evaluation NLL than both Dense and the learned
contextual router in all four local runs (Table~\ref{tab:mps}). Its mean paired
improvement over the learned router is 0.061159 NLL for GQA and 0.090491 for
MHA. The learned router is slightly behind Dense in three runs and ahead by
0.060878 NLL for MHA seed 43. The substantial variation between MHA seeds
cautions against interpreting either mean as a precise effect estimate.
Current median logged MPS throughput is lower for fixed routed models: 4708
versus 5418.5 tokens/s for GQA, and 4398.5 versus 5590.5 tokens/s for MHA at
seed 42. These fallback measurements are implementation-specific and are not
CUDA deployment benchmarks.
...
an important longer-budget control: the four short local comparisons do not
establish that fixed routing is generally superior to learned routing.
...
short-budget direction under both GQA and MHA, including against a learned
contextual-router control, subject to much stronger limitations. The evidence
motivates controlled replication rather than a claim of a universally better
