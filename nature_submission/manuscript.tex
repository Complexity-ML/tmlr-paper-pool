...
\begin{abstract}
Mixture-of-experts language models usually learn routers from contextual hidden states. Within a grouped-query-attention transformer, we test a fixed feed-forward primitive in which token identity selects narrow residual experts while a shared dense multilayer perceptron preserves contextual processing. In a matched single-seed comparison, 306.5-million-parameter dense and token-routed models were trained on 8 billion FineWeb-Edu tokens. Routing reached diagnostic-stream NLL 2.9329 versus 2.9482 for dense, but trained 21\% more slowly. Final checkpoints were near parity on three zero-shot tasks. Routing had lower WikiText-2 perplexity and C4-validation NLL, whereas dense had lower NLL on a Pile test subset. A learned-router control also changed ordering between short and 1-billion-token budgets. Fixed token identity therefore allocated useful residual capacity, but the advantage was modest and corpus-dependent. Multi-seed and compute-matched validation remains necessary.
\end{abstract}
...
We test that possibility in a deliberately asymmetric architecture. Every token passes through a shared SwiGLU multilayer perceptron (MLP). Its token identifier selects two additional narrow experts, which receive the same contextual hidden state and add a gated residual. Thus token identity selects functions but does not replace contextual representations. We evaluate this primitive inside a decoder-only transformer with causal grouped-query attention (GQA), keeping the sequence-processing backbone identical so that the routing effect is isolated from changes to sequence modelling. The separation between a shared contextual substrate and fixed residual access is not conceptually tied to attention, but the present experiments do not test backbone independence. The primary experiment compares this architecture with a parameter-matched dense model at 306.5 million parameters and 8 billion training tokens. We also evaluate the final checkpoints on four standard tasks and report separate 100-million-parameter controls with fixed, dense and learned contextual routing.

The token-routed model crosses the dense training curve after approximately 0.8 billion tokens and retains a small negative-log-likelihood (NLL) advantage on the FineWeb-Edu diagnostic stream. It is slower in the present implementation, and task accuracy differences are smaller than one reported standard error. Independent-corpus NLL favours routing on C4 validation but dense on a Pile test subset. The evidence therefore supports a narrow and distribution-dependent effect rather than a general ranking of routing mechanisms.
...
\end{equation}
Thus the second route is the cyclic successor of the primary route. Both mappings are deterministic, layer specific and independent of token frequency.

\begin{figure}[t]
\centering
\includegraphics[width=0.98\textwidth]{architecture_complexity_deep.pdf}
\caption{\textbf{Fixed token-identity routing controls residual parameter selection within a GQA transformer.} \textbf{a}, The controlled decoder-only backbone uses causal grouped-query attention (GQA) to produce a contextual hidden state $\mathbf{x}$. Every token traverses the shared SwiGLU branch, while its token identifier selects two narrow residual experts through a fixed layer-specific table. \textbf{b}, Different contextual states associated with the same token select the same expert pair but produce different residual outputs. Token identity therefore fixes parameter selection, not the contextual computation performed by those parameters.}
...
\end{equation}
where $g_l^s$ and $g_l^r$ are learned scalar gates initialized to 0.5 and 0.5. Shared and routed branches are SwiGLU transformations. No auxiliary expert objective is used in the primary token-identity experiment.
...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{fig_300m_loss_curves.pdf}
\caption{\textbf{Matched-token loss measurements.} \textbf{a}, Training NLL at recorded checkpoints, with the post-crossover region enlarged in the inset. The token-identity residual model (orange) begins behind dense (blue) and first crosses near 0.8 billion tokens. \textbf{b}, Token-identity residual minus dense NLL on the fixed FineWeb-Edu training-split diagnostic stream; negative values favour token routing. The final common evaluation differs by $-0.0153$ NLL after 7.864 billion tokens. Markers are recorded checkpoints and lines guide the eye. The diagnostic stream is not held out, and each curve represents one seed.}
\label{fig:training}
\end{figure}
...
\subsection*{Standard and independent-corpus evaluations are mixed}
...
We additionally scored two independently selected corpus subsets with the exact checkpoint tokenizer and the same Apple MLX implementation. Each subset contains 262,144 causal-LM targets in 128 contiguous 2,048-token blocks. On C4 validation\cite{raffel2020exploring}, token routing has NLL 3.4066 (perplexity 30.16) versus 3.4161 (30.45) for dense; the paired block-bootstrap difference is $-0.0095$ (95\% confidence interval, $-0.0127$ to $-0.0063$). On The Pile test split\cite{gao2020pile}, dense has NLL 2.8690 (17.62) versus 2.8769 (17.76) for token routing; the difference is $+0.0079$ ($+0.0022$ to $+0.0138$). No decontamination against FineWeb-Edu was performed. The ordering therefore changes by corpus.
...
These experiments isolate a modest effect: a fixed token identifier can select a useful residual parameter subspace while contextual computation remains shared. The shared-context/token-identity separation is therefore an architectural building block, not a demonstrated replacement for the transformer backbone. It could in principle be paired with another contextual substrate, but that possibility requires direct experiments. The result does not imply that token identifiers encode semantic expert classes. The primary assignments are modulo partitions up to a layer-specific permutation, and every active expert transforms the contextual hidden state. Stable token-conditioned access to parameters is sufficient to explain the observation.
...
The largest limitations are statistical and evaluative. Each architecture has one training seed, the training-time loss stream comes from the training split, the standard task panel is small, and each independent-corpus estimate uses only 262,144 tokens without decontamination. The opposite C4 and Pile orderings show that the effect is distribution dependent. Replication across seeds, larger preregistered held-out samples, stronger out-of-distribution tasks and larger models are needed. A matched compute or wall-clock study is also required to evaluate efficiency.
...
Each layer uses a seeded permutation of four expert indices. Primary routes are obtained from the permuted token-ID residue, and the second route is the cyclic successor of the primary expert. With a 32,000-entry vocabulary, every expert receives 8,000 primary identifiers and 8,000 secondary identifiers. Checkpoint inspection verified the stored primary lookup buffers in all 18 layers; the historical training code and exported evaluation implementation both derive the cyclic second route from those buffers. The construction uses no token counts or frequency table.
...
The historical fixed-routing panel consists of seven approximately 100-million-parameter models trained for 1.0003 billion tokens on two NVIDIA B200 GPUs. It tests a dense residual MLP, a shared-only branch, a routed-only branch, random assignment and three deterministic top-2 labels. Preserved configurations and launcher source show that the condition historically configured as frequency-aware had no populated frequency table and realized modulo-primary/cardinality-balanced-secondary assignment. Some named controls have equivalent primary assignments; differences among them are descriptive. The launcher retained no checkpoints, so this panel is reported from raw metrics and requires rerunning before new corpus evaluation.
...
We streamed the first documents from pinned versions of English C4 validation (revision \texttt{1588ec4}) and \texttt{EleutherAI/pile\_val\_test} test (revision \texttt{05b3270}). Documents were joined with EOS and packed using the checkpoint's exact 32,000-entry tokenizer. For each corpus, 128 blocks of 2,048 targets were scored once by each checkpoint with Apple MLX/Metal; no weights were updated. C4 consumed 529 documents and The Pile 191 documents. These corpora are distinct from the declared FineWeb-Edu training stream, but we did not perform document-level decontamination. Full revisions, hashes, per-block NLLs and runtime versions are included in the supplementary archive.

\subsection*{Statistics and reproducibility}

Training experiments use one seed per architecture; no confidence interval or hypothesis test is reported for training loss. Multiple-choice standard errors are returned by the evaluation harness. Independent-corpus confidence intervals use 10,000 paired bootstrap resamples over the 128 fixed token blocks with seed 20,260,728. Source metrics, per-block values, configurations, route-verification tests and figure scripts accompany the supplementary code.
...
FineWeb-Edu, C4 and The Pile are publicly available under their applicable repository terms. Exact dataset revisions, tokenizers, preprocessing instructions, run configurations, aggregate results and independent-corpus per-block values are included in the anonymized peer-review archive. The full checkpoints are available to editors and reviewers on request and will be deposited in a persistent public repository upon publication.

\section*{Code availability}

An anonymized source archive containing the PyTorch implementation, fixed and learned routing variants, route tests, training entry points, Apple MLX scorer and analysis scripts accompanies the submission. A public, versioned repository and archival release will be provided upon publication.
