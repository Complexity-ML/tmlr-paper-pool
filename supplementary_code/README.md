...
- `scripts/render_verified_architecture.py` and `scripts/architecture_complexity_deep.dot`: regenerate the vector architecture diagram from its Graphviz source.
- `scripts/render_300m_scaling_figure.py`: regenerates the paper's separated training-stream and fixed-evaluation-stream figure from that summary.
