...
To assess the contribution of each architectural component, we perform inference-time ablations on the pre-trained BASE model. Each component is individually disabled by zeroing out its corresponding weights, while keeping all other components intact. We evaluate on 200 samples per benchmark.
...
\midrule
\multicolumn{7}{l}{\textit{$\Delta$ vs Full Model:}} \\
No Mu-Guidance & +1.0 & +1.0 & $-$1.0 & $-$7.5 & +4.0 & $-$0.5 \\
No Token-Routing & $-$0.5 & +1.0 & +2.0 & $-$0.5 & $-$1.5 & +0.1 \\
No PiD Controller & +3.5 & +3.0 & +2.5 & $-$2.5 & $-$3.5 & +0.6 \\
\bottomrule
\end{tabular}
\caption{Inference-time ablation results (\%). Bold values indicate notable drops when a component is removed.}
\label{tab:ablations}
\end{table}

\textbf{Analysis.} Several observations emerge from these inference-time ablations:

\begin{enumerate}
    \item \textbf{Mu-Guidance impact on reasoning}: Disabling Mu-Guidance causes the largest single-benchmark drop: ARC-Easy falls by $-$7.5\%, suggesting that bidirectional information flow is most beneficial for scientific reasoning tasks where top-down context aids answer selection.

    \item \textbf{Token-Routing robustness}: The minimal impact of disabling Token-Routing at inference is \textit{expected}: since model weights were optimized under modulo routing, expert weights already encode routing-specific specializations. The true contribution of Token-Routing is architectural (compute reduction by factor $n$, see Theorem~\ref{thm:capacity}) rather than measurable through inference-time ablation.

    \item \textbf{PiD Controller as training stabilizer}: The PiD Controller's primary role is during training (gradient stability, convergence), not inference. Its removal at inference allows slightly different activation dynamics that may incidentally improve some benchmarks while degrading others (Winogrande: $-$3.5\%).

    \item \textbf{Methodology note}: These are inference-time ablations on a model trained with all components active. This methodology provides a \textit{lower bound} on component importance, as weights co-adapt during training. Full training-time ablations (retraining from scratch without each component) would provide more definitive evidence and are planned as future work.
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{Training-time ablations}: While our inference-time ablations (Section~\ref{sec:ablations}) provide initial evidence, full training-time ablations (retraining from scratch without each component) are needed to definitively quantify each innovation's contribution
...
Inference-time ablation studies reveal that Mu-Guidance has the strongest impact on reasoning benchmarks (ARC-Easy: $-$7.5\% when disabled), while Token-Routing and PiD Controller contributions are primarily architectural (compute efficiency) and training-related (stability), respectively. Future work will include: (1) training-time ablations to definitively quantify component contributions, and (2) rigorous validation of the Core Dependency Hypothesis through controlled experiments.
