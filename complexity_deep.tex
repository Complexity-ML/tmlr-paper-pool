...
A shared dense MLP processes \textit{all} tokens, capturing universal patterns (syntax, grammar), while routed experts specialize on their lexical subsets. The output combines both through a learnable gate $\gamma \in \mathbb{R}$:
\begin{equation}
\text{MLP}_{\text{output}}(\mathbf{x}) = \underbrace{\text{SwiGLU}_{\text{shared}}(\mathbf{x})}_{\text{common patterns}} + \gamma \cdot \underbrace{\text{SwiGLU}_{e}(\mathbf{x})}_{\text{lexical specialization}}
\end{equation}
...
\paragraph{Routed gate $\gamma$.} The learnable scalar $\gamma$ (initialized to $0.1$) controls the progressive contribution of routed experts. With $\gamma=0$ the model reduces to a dense SwiGLU with Zipf-routed experts kept as reserve capacity, recovering pure dense convergence in early training. As $\gamma$ grows via gradient descent, the routed branch contributes an increasing share of the update, enabling specialization once the shared backbone has stabilized. Empirically $\gamma$ rises monotonically during pretraining (from $0.10$ to $\sim$$0.15$--$0.30$ after $10$k steps on our setup), indicating the experts progressively learn useful residuals that the shared branch does not capture.
