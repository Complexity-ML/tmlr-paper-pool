...
\paragraph{Ablation (500M tokens).} These 500M-token models serve solely to compare architectural components via ablation, not to evaluate final performance. Over 954 steps, three iso-param ($\sim$170--187M) configurations are compared: dense (SwiGLU), full Token-Routed (Shared Expert + Mu-Guidance + Zipf), and Token-Routed without Mu-Guidance.

\begin{table}[h]
\centering
\caption{Mu-Guidance ablation (500M tokens, 954 steps, $\sim$171M models). Average loss over the full training measures overall convergence speed.}
...
The full model (Run 2) converges significantly faster than the dense baseline ($-0.112$ in average loss). Without Mu-Guidance (Run 3), the Token-Routed model is slightly slower than dense ($+0.011$), confirming that the inter-layer equilibrium signal is the key architectural component.

\paragraph{Inference performance.} The Run 2 model (187M) deployed on vLLM with PagedAttention and CUDA graphs achieves an average throughput of \textbf{204 tokens/s} over a 2,040-token output context (NVIDIA RTX 5060 Ti GPU, 16 GB). The deterministic per-token routing (no learned router) is natively compatible with CUDA graph capture, eliminating the CPU--GPU synchronizations required by classical MoE architectures.
...
\begin{itemize}
    \item \textbf{8B-token run}: Preliminary results (500M tokens) show a Token-Routed advantage in fast convergence, but the definitive 8B-token results are in preparation to observe the dense plateau vs continued TR descent
    \item \textbf{No-Mu ablation}: A Token-Routed + Shared + Zipf run \textit{without} Mu-Guidance is needed to isolate Mu's contribution from other improvements
    \item \textbf{Standardized benchmarks}: Evaluation on MMLU, ARC, HellaSwag using \texttt{lm-evaluation-harness} is planned
    \item \textbf{Scaling}: Current ablations are at 166--187M; validations at 400M+ would strengthen the claims
\end{itemize}
