...
An earlier version of the architecture included a PiD-style Dynamic Scaler that modulated residual contributions via a learned controller. Empirical results at both 100M and 170M proxy scales showed inconsistent benefits: at 100M, removing PiD \textit{improved} loss by $-0.005$, while at 170M it contributed only $+0.010$. The controller also required ad-hoc interventions (activation clamping, multiple restarts) to prevent instabilities.
...
\begin{table}[h]
\centering
\caption{Iso-parameter ablation configurations ($\sim$166--176M, 8B tokens FineWeb-Edu).}
...
\paragraph{Ablation (500M tokens).} These 500M-token models serve solely to compare architectural components via ablation, not to evaluate final performance. Over 954 steps, three iso-param ($\sim$170--176M) configurations are compared: dense (SwiGLU), full Token-Routed (Shared Expert + Mu-Guidance + Zipf), and Token-Routed without Mu-Guidance.

\begin{table}[h]
\centering
\caption{Mu-Guidance ablation (500M tokens, 954 steps, $\sim$170M models). Average loss over the full training measures overall convergence speed.}
...
Run 3: TR + Shared + Zipf (no Mu)        & 176M & 4.916 & $+0.011$ \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{Inference performance.} The Run 2 model (176M) deployed on vLLM with PagedAttention and CUDA graphs achieves an average throughput of \textbf{204 tokens/s} over a 2,040-token output context (NVIDIA RTX 5060 Ti GPU, 16 GB). The deterministic per-token routing (no learned router) is natively compatible with CUDA graph capture, eliminating the CPU--GPU synchronizations required by classical MoE architectures.

\begin{figure}[!htbp]
\centering
\includegraphics[width=0.9\textwidth]{../figures/fig_loss_curves.png}
\caption{Loss curves: Dense 170M vs full Token-Routed (Shared+Mu+Zipf) 176M vs Token-Routed without Mu-Guidance 176M (500M tokens). The full model (blue) converges faster throughout. Without Mu-Guidance (orange), the MoE is slower than the dense baseline, confirming the essential role of the equilibrium signal.}
...
\begin{itemize}
    ...
    \item \textbf{Scaling}: Current ablations are at 166--176M; validations at 400M+ would strengthen the claims
\end{itemize}
