...
\paragraph{Preliminary results (500M tokens).} Over 954 steps, the Token-Routed model converges faster (average loss 4.793 vs 4.905 for dense---advantage of $-0.112$). The dense model catches up after step 600 thanks to its larger per-token capacity (2416 vs 1024 active dims). The full 8B-token run will determine whether the dense model saturates (over-trained at 242\% Chinchilla) while the Token-Routed---where each expert sees only 25\% of tokens---continues to descend.

\begin{table}[h]
\centering
\caption{Ablation results (500M tokens, 954 steps). Token-Routed leads over the first 600 steps.}
\label{tab:ablation_results}
\begin{tabular}{lccc}
\toprule
\textbf{Run} & \textbf{Avg Loss (954 steps)} & \textbf{Final Loss} & \textbf{Final PPL} \\
\midrule
...
\caption{Loss curves: Dense 170M vs Token-Routed + Shared 176M (500M tokens). The TR converges faster over the first 600 steps.}
