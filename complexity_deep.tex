...
\begin{equation}
\mu^{(l)}_{\text{contextual}} = \boldsymbol{\mu}^{(l)}_{\text{param}} + \mathbf{W}_{\mu}^{(l)} \mathbf{h}^{(l)}_{\text{post-MLP}}
\label{eq:mu_update}
\end{equation}
where $\boldsymbol{\mu}^{(l)}_{\text{param}} \in \mathbb{R}^{d}$ is a learnable parameter \textit{specific to layer $l$} (initialized to zero) and $\mathbf{W}_{\mu}^{(l)}$ is a contextual linear projection (initialized to zero). This $\mu^{(l)}_{\text{contextual}}$ becomes the $\mu^{(l-1)}$ used by layer $l+1$ in Equations~\ref{eq:key}--\ref{eq:value}. We do not clamp $\mu$: with $\mathbf{W}_{\mu}^{(l)}$ initialized to zero, the autoregressive feedback builds up gradually, and the Lipschitz bound of Section~\ref{sec:theory} still holds since it depends on $\|\mathbf{W}_{\mu}^{(l)}\|$, not on the range of $\mu$ itself.
...
A shared dense MLP processes \textit{all} tokens, capturing universal patterns (syntax, grammar), while routed experts specialize on their lexical subsets. The output combines both additively:
\begin{equation}
...
\end{align}
The shared expert has the full intermediate size $d_{ff}$, matching a dense SwiGLU; each routed expert has intermediate size $d_{ff}/n$. This sizing ensures the shared branch alone is iso-compute with a dense baseline, while the routed experts provide additional specialization capacity (adding $\sim$25\% parameters over dense). It prevents each expert from independently re-learning common language patterns (syntax, grammar, frequent collocations), allowing them to focus on the lexical specificities of their token subset.

The combination is a simple residual sum: both branches write into the residual stream with equal weight. This avoids learnable gating scalars which, under mixed-precision (BF16) training, suffer from numerical underflow on their gradients and stall at initialization.
...
\paragraph{GPT-style initialization.} Residual output projections (\texttt{o\_proj}, routed \texttt{down\_proj}, and the shared expert's \texttt{shared\_down}) are initialized with reduced standard deviation:
\begin{equation}
\sigma_{\text{residual}} = \frac{0.02}{\sqrt{2 \times L}}
\end{equation}
where $L$ is the number of layers. This prevents the residual stream from growing with depth~\citep{radford2019language}. We extend the scaling to \texttt{shared\_down} (missing in the reference GPT-2 recipe which has no shared branch) because it is a residual output projection in our architecture.
...
\paragraph{Training efficiency.} At 384M scale on 4$\times$ NVIDIA B200 (8B tokens, batch 128, BF16 FSDP), the Token-Routed model trains at \textbf{1.61\,s/step} while the iso-parameter dense baseline requires \textbf{2.52\,s/step} --- a \textbf{1.57$\times$} wall-clock speedup per step. Total training time: 3\,h\,28\,min (MoE) vs $\sim$5\,h\,20\,min (dense). The speedup comes from the MoE's lower active compute per token: the shared expert processes all tokens through $d_{ff}$ while each routed expert adds only $d_{ff}/n$, giving a total active width of $d_{ff} + d_{ff}/n$ vs $d_{ff,\text{dense}}$ for the dense baseline at equal parameter count.
